 ...
 * $H_{a}$: There is a significant association between the occurrence of
   diabetes and racial groups.
...
dr_xsq <- dr_test$statistic

```

Using a $\chi^{2}$-Square test to see if there is a significant difference
between the observed cell counts of presence of diabetes among individuals in
different racial groups and expected cell counts under the null hypothesis
produces a test statistics of `r round(dr_xsq, 3)`. Given the test statistic
is greater that $0$ and has an associated _P_-value of `r round(dr_pval, 3)`
(_< .05_), we reject the null hypothesis that there is no significant
association between diabetes and different racial groups.
...
In model _A_, the intercept represents the estimated population mean creatinine
level of individuals who survived at least $5$ years. The slope of model _A_
represents the estimated difference in population means between the population
of individuals who dies within $5$ years and the population which survived at
least $5$ years.
...
surv_inf_tbl <- surv_b_tidy[2, ] %>%
  select(`Point est.` = estimate, `Std. error` = std.error, `P-val` = p.value) %>%
  bind_cols(as_tibble(t(surv_b_ci[2, ]))) %>%
...
knitr::kable(
  ...
  booktabs = TRUE,
  digits = 3,
  caption = paste("Inference table for the difference in mean creatinine level",
                  "between a population that survived 5+ years, and a",
                  "population that died within 5 years"
                  )
)

```

From looking at our $95\%$ confidence interval (which doesn't include $0$) and
our _P_-value (which is _< .05_), we can conclude that there is a statistically
significant association between mean creatinine level and survivorship status
at 5 years.
